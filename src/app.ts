...
// (async () => {
  ...
  puppeteer.use(pluginAnonymizeUA());
   puppeteer.launch({ headless: false, slowMo: 100, protocolTimeout: 70000,args: ["--no-sandbox"],executablePath:"C:/Program Files/Google/Chrome/Application/chrome.exe" }).then(async (browser) => { 
      ...
      try {
        const cookiesFilePath:string = path.join(__dirname,'Cookies/cookies.json');
      ...
      await page.keyboard.press("Enter");
        ...
        for(let i=0;i<5;i++)
        {
            ...
            await page.waitForSelector(".artdeco-pagination__button.artdeco-pagination__button--next", { timeout: 10000 });
